...
class TaskListViewTest(TestCase):
    ...
    def test_task_update_view_status_code(self):
        task = Task.objects.create(
            content="Test Task", created_at=timezone.now()
        )
        response = self.client.get(reverse(
            "todo_app:task-update", kwargs={"pk": task.pk})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_task_delete_view_status_code(self):
        ...
        response = self.client.get(reverse(
            "todo_app:task-delete", kwargs={"pk": task.pk})
        )
        self.assertEqual(response.status_code, 200)
...
class TagListViewTest(TestCase):
    ...
    def test_tag_update_view_status_code(self):
        ...
        response = self.client.get(reverse(
            "todo_app:tag-update", kwargs={"pk": tag.pk})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_tag_delete_view_status_code(self):
        tag = Tag.objects.create(name="Test Tag")
        response = self.client.get(reverse(
            "todo_app:tag-delete", kwargs={"pk": tag.pk})
        )
        self.assertEqual(response.status_code, 200)
